...
@Stateful
@LocalBean
public class ShipmentConfirmedExecutor extends AbstractEventExecutor<ShipmentConfirmedEvent> {
	...
	public String getEventId() {
		return ShipmentConfirmedEvent.class.getName();
	}
	...
	public void cancel(String correlationId) {
		...
		String reason = "ShipmentConfirmed execution cancelled";
		getProcess().handle_ShipmentConfirmed_event_cancel(correlationId, reason);
		log.info("cancelled");
	}
	...
	public void process(@Observes ShipmentConfirmedEvent event) {
		log.info("invoked");
		shutdownTimer();
		...
		try {
			getProcess().handle_ShipmentConfirmed_event(event);
			log.info("processed");
			...
			getProcess().handle_ShipmentConfirmed_event_exception(correlationId, e);
	...
	protected TimerTask createTimeoutHandler(final String correlationId) {
		return new TimerTask() {
			public void run() {
				reset();
				log.info("timed-out");
				String reason = "ShipmentConfirmed timed-out";
				getProcess().handle_ShipmentConfirmed_event_timeout(correlationId, reason);
			}
		};
	}
