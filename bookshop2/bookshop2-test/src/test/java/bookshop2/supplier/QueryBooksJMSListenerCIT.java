...
public class QueryBooksJMSListenerCIT extends AbstractJMSListenerArquillionTest {
	...
	public Class<?> getTestClass() {
		return QueryBooksJMSListenerCIT.class;
	}
...
//	}
	...
	@Test
	@InSequence(value = 1)
	public void testQueryBooks_BooksAvailable() throws Exception {
		String testName = "testQueryBooks_BooksAvailable";
		...
		Assert.isTrue(isFiredRequestDone("QueryBooks"));
		Assert.isTrue(isFiredResponseSent("BooksAvailable"));
		Assert.isFalse(isFiredResponseSent("BooksUnavailable"));
	...
	public void testQueryBooks_BooksUnavailable() throws Exception {
		...
		Assert.isTrue(isFiredRequestDone("QueryBooks"));
		Assert.isFalse(isFiredResponseSent("BooksAvailable"));
		Assert.isTrue(isFiredResponseSent("BooksUnavailable"));
		log.info(testName+": done");
		if (errorMessage != null)
			fail(errorMessage);
	}
	...
	protected void registerNotificationListeners() throws Exception {
		addRequestNotificationListeners("QueryBooks");
		addResponseNotificationListeners("BooksAvailable");
		addResponseNotificationListeners("BooksUnavailable");
	}
