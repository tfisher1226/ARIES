...
public class ClientInterceptorProvider extends AbstractManagementBeanProvider {

	private Service service;

	
	public ClientInterceptorProvider(GenerationContext context) {
		super(context);
	}
	...
	protected void generateSourceCode_ProxyInvocation(ModelOperation modelOperation, Element element, String serviceName) {
		...
		//String operationName = getUniqueOperationName(modelOperation);
		String sourceId = application.getName();
		if (service instanceof Callback == false)
			sourceId += "-client";
		buf.putLine2("	log.info(\"#### ["+sourceId+"]: "+operationName+"() sending\");");
		
		if (ServiceUtil.isSynchronous(service))
			buf.putLine2("	Message request = createMessage(\""+operationName+"\");");
		else buf.putLine2("	Message message = createMessage(\""+operationName+"\");");
		...
		while (iterator.hasNext()) {
			...
			else buf.putLine2("	message.addPart(\""+name+"\", "+name+");");
		}
		...
		if (resultType != null && !resultType.equals("void")) {
			...
			if (ServiceUtil.isSynchronous(service))
				buf.putLine2("	getProxy().send(request);");
			else buf.putLine2("	getProxy().send(message);");
		}
	...
	public static String getUniqueOperationName(ModelOperation operation) {
		String operationName = operation.getName();
		List<ModelParameter> parameters = operation.getParameters();
		if (parameters.size() == 1) {
			ModelParameter parameter = parameters.get(0);
			String structure = parameter.getConstruct();
			String name = parameter.getName();
			operationName = OperationUtil.getUniqueOperationName(operationName, structure, name);
		}
