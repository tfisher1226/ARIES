...
/**
 * Builds a RMI Service Client (i.e. client-side) Implementation {@link ModelClass} object given a {@link Service} Specification as input;
 * 
 ...
 * @author tfisher
 */
public class ClientClassRMIBuilder extends AbstractManagementBeanBuilder {

	private ClientClassJMSProvider provider;

	
	public ClientClassRMIBuilder(GenerationContext context) {
		super(context);
		initialize();
	}
	...
	public ModelClass build(Service service) throws Exception {
		...
		String className = ClientLayerHelper.getClientInterfaceName(service) + "ProxyForRMI";
		...
		modelClass.setParentClassName("RMIProxy");
		modelClass.addImportedClass("org.aries.tx.service.rmi.RMIProxy");
