package nam.model.repository;

import java.io.Serializable;
import java.util.Collection;
import java.util.List;

import javax.enterprise.context.SessionScoped;
import javax.inject.Inject;
import javax.inject.Named;

import nam.model.Repository;
import nam.model.util.RepositoryUtil;
import nam.ui.design.SelectionContext;


@SessionScoped
@Named("repositoryDataManager")
public class RepositoryDataManager implements Serializable {
	
	@Inject
	private RepositoryEventManager repositoryEventManager;
	
	@Inject
	private SelectionContext selectionContext;
	
	private String scope;
	
	
	public String getScope() {
		return scope;
	}
	
	public void setScope(String scope) {
		this.scope = scope;
	}
	
	protected <T> T getOwner() {
		T owner = selectionContext.getSelection(scope);
		return owner;
	}
	
	public Collection<Repository> getRepositoryList() {
		if (scope == null)
			return null;
		
		Object owner = getOwner();
		if (owner == null)
			return null;
		
		return getDefaultList();
	}
	
	public Collection<Repository> getDefaultList() {
		return null;
	}
	
	public void saveRepository(Repository repository) {
		if (scope != null) {
			Object owner = getOwner();
		}
	}
	
	public boolean removeRepository(Repository repository) {
		if (scope != null) {
			Object owner = getOwner();
		}
		return false;
	}
	
}
