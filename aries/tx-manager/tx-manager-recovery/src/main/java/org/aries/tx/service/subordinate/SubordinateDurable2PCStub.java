...
public class SubordinateDurable2PCStub implements Durable2PCParticipant {
	...
	public void commit() throws WrongStateException, SystemException {
		if (!isRecovered()) {
			...
			//TODO XTSATRecoveryManager recoveryManager = null;
			boolean isRecoveryScanStarted = false;
			if (coordinator == null) {
				// try fetching coordinator from the recovery manager
				//TODO recoveryManager = XTSATRecoveryManagerImpl.getRecoveryManager();
				// check whether recovery has started before we check for the presence
				// of the subordinate coordinator
				//TODO isRecoveryScanStarted = recoveryManager.isSubordinateCoordinatorRecoveryStarted();
	...
	public void rollback() throws WrongStateException, SystemException {
		if (!isRecovered()) {
			...
			//TODO XTSATRecoveryManager recoveryManager = XTSATRecoveryManagerImpl.getRecoveryManager();
			//TODO boolean isRecoveryScanStarted = recoveryManager.isSubordinateCoordinatorRecoveryStarted();
			// now look for a subordinate coordinator with the right id
			coordinator = SubordinateATCoordinator.getRecoveredCoordinator(coordinatorId);
			if (coordinator == null) {
				//TODO if (!isRecoveryScanStarted) {
				//TODO 	// the subtransaction may still be waiting to be resolved
				//TODO 	// throw an exception causing the rollback to be retried later
