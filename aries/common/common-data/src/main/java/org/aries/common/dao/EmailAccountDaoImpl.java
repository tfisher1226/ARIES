...
@Stateless
@Local(EmailAccountDao.class)
public class EmailAccountDaoImpl<T extends EmailAccountEntity> extends DaoImpl<T> implements EmailAccountDao<T> {
	...
	public T getEmailAccountRecordByUserId(String userId) {
		...
		Query query = entityManager.createNamedQuery("get"+name+"RecordByUserId");
	...
	public Long addEmailAccountRecord(T emailAccountRecord) {
		Assert.notNull(emailAccountRecord, "EmailAccount record parameter must be specified");
		...
		try {
			...
			T merged = entityManager.merge(emailAccountRecord);
			entityManager.persist(merged);
			transactionHelper.close();
		
			Long id = merged.getId();
			emailAccountRecord.setId(id);
			return id;
	...
	public List<Long> addEmailAccountRecords(Collection<T> emailAccountRecords) {
		Assert.notNull(emailAccountRecords, "EmailAccount records parameter must be specified");
		TransactionHelper transactionHelper = new TransactionHelper();
		...
		try {
			...
			Iterator<T> iterator = emailAccountRecords.iterator();
			while (iterator.hasNext()) {
				T emailAccountRecord = iterator.next();
				T merged = entityManager.merge(emailAccountRecord);
				entityManager.persist(merged);
				Long id = merged.getId();
				emailAccountRecord.setId(id);
	...
	public void saveEmailAccountRecords(Collection<T> emailAccountRecords) {
		Assert.notNull(emailAccountRecords, "EmailAccountRecords parameter must be specified");
		TransactionHelper transactionHelper = new TransactionHelper();
