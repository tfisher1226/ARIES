...
@Stateless
@Local(StreetAddressDao.class)
public class StreetAddressDaoImpl<T extends StreetAddressEntity> extends DaoImpl<T> implements StreetAddressDao<T> {
	...
	public Long addStreetAddressRecord(T streetAddressRecord) {
		Assert.notNull(streetAddressRecord, "StreetAddress record parameter must be specified");
		...
		try {
			...
			T merged = entityManager.merge(streetAddressRecord);
			entityManager.persist(merged);
			transactionHelper.close();
		
			Long id = merged.getId();
			streetAddressRecord.setId(id);
			return id;
	...
	public List<Long> addStreetAddressRecords(Collection<T> streetAddressRecords) {
		Assert.notNull(streetAddressRecords, "StreetAddress records parameter must be specified");
		TransactionHelper transactionHelper = new TransactionHelper();
		...
		try {
			...
			Iterator<T> iterator = streetAddressRecords.iterator();
			while (iterator.hasNext()) {
				T streetAddressRecord = iterator.next();
				T merged = entityManager.merge(streetAddressRecord);
				entityManager.persist(merged);
				Long id = merged.getId();
				streetAddressRecord.setId(id);
	...
	public void saveStreetAddressRecords(Collection<T> streetAddressRecords) {
		Assert.notNull(streetAddressRecords, "StreetAddressRecords parameter must be specified");
		TransactionHelper transactionHelper = new TransactionHelper();
