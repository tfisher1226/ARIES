...
public class EJBMessageInvoker<T> implements MessageInvoker<T> {
    ...
    public EJBMessageInvoker(EJBEndpointContext context) {
	...
	public Message invoke(Message request, String correlationId, long timeout) {
		...
		String responseXml = client.invoke(requestXml, correlationId, timeout);
	...
	public Future<Message> invoke(Message request, String correlationId, Handler<T> handler) {
		...
		String requestXml = context.toXML(request);
		client.invoke(requestXml, correlationId, delegator);
		//TODO check for exception and throw it
		return future;
	}
...
//				
//				String xml = client.receive(messageTimeout);
//				Message message = context.fromXML(xml);
