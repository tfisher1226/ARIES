...
public abstract class AbstractArquillianWARBuilder {

	//Make empty web.xml by default
	public StringAsset getWebXml() {
		WebXmlBuilder builder = new WebXmlBuilder();
		builder.setDisplayName("Default Web XML");
		String xml = builder.createWebXml();
		StringAsset asset = new StringAsset(xml);
		return asset;
	}

	public WebArchiveBuilder createWebArchiveBuilder(String fileName) {
		WebArchiveBuilder builder = new WebArchiveBuilder(fileName);
		return builder;
	}
	
	public void addDefaultTestLibraries(WebArchiveBuilder builder) {
	}
	
	public void addOtherLibraries(WebArchiveBuilder builder) {
		//add local libraries
